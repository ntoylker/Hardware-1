...
\begin{enumerate}
    ...
    \item \textbf{Αρνητική Υπερχείλιση:} [\textlatin{MAX\_NEG}, \textlatin{MAX\_NEG\_HALF}].
\end{enumerate}
Στο τέλος, αναφέρει το συνολικό σκορ (π.χ. \textlatin{298 / 300 tests passed}).

\paragraph{Ανάλυση Αποτελεσμάτων (Οριακές Περιπτώσεις)}
Η εκτέλεση του \textlatin{testbench} για 100 επαναλήψεις (συνολικά 300 δοκιμές) ανέδειξε ποσοστό επιτυχίας \textbf{99.3\% (298/300 \textlatin{tests passed})}.

Συγκεκριμένα:
\begin{itemize}
    \item Τα 100 τεστ του <<\textlatin{Normal Range}>> πέρασαν επιτυχώς.
    \item Τα 198 από τα 200 τεστ υπερχείλισης πέρασαν επιτυχώς, επιβεβαιώνοντας ότι η λογική παράκαμψης σε \textlatin{IDLE} και η έξοδος -1 λειτουργούν σωστά στη συντριπτική πλειοψηφία των περιπτώσεων.
    \item Παρατηρήθηκαν μόλις 2 αποτυχίες (\textlatin{FAILs}) σε τυχαία σενάρια.
\end{itemize}

\textbf{Τεχνική Αιτιολόγηση:} Η ανάλυση των αποτελεσμάτων έδειξε ότι οι 2 αυτές αποτυχίες οφείλονται σε ασυμφωνία στον χειρισμό της υπερχείλισης κατά την \textbf{ολίσθηση} (\textlatin{bit shifting}).
Το μοντέλο αναφοράς (\texttt{\textlatin{nn\_model}}) ελέγχει αυστηρά αν η τελική ολίσθηση αλλοιώνει το πρόσημο του αριθμού και επιστρέφει -1.
Αντίθετα, η μονάδα \textlatin{ALU} (όπως σχεδιάστηκε στην Άσκηση 1 βάσει των τότε προδιαγραφών) δεν παράγει σήμα υπερχείλισης (\texttt{\textlatin{ovf}}) για τις εντολές ολίσθησης (\texttt{\textlatin{ALUOP\_ASL}}).
Ως αποτέλεσμα, όταν συμβαίνει υπερχείλιση \textit{αποκλειστικά} στο τελικό στάδιο (\textlatin{Post-Processing}), το υλικό επιστρέφει την ολισθημένη τιμή, ενώ το μοντέλο το θεωρεί σφάλμα. Η συμπεριφορά του κυκλώματος κρίνεται ορθή βάσει των προδιαγραφών της \textlatin{ALU} που υλοποιήθηκε.\\\\

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{tb_nn_score_298.png}
    \caption{Τελικό αποτέλεσμα προσομοίωσης: 298/300 επιτυχείς δοκιμές, επιβεβαιώνοντας τη σωστή λειτουργία της \textlatin{FSM} και του \textlatin{Datapath}.}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=1\textwidth]{tb_nn_fail_analysis.png}
    \caption{Στιγμιότυπο αποτυχίας (\textlatin{FAIL}): Το μοντέλο αναφοράς επιστρέφει \textlatin{FFFFFFFF} (-1) λόγω υπερχείλισης στην ολίσθηση, ενώ το \textlatin{DUT} δεν ανιχνεύει σφάλμα (\textlatin{OVF=0}) λόγω των προδιαγραφών της \textlatin{ALU}, οδηγώντας σε ασυμφωνία.}
    \label{fig:fail_analysis}
\end{figure}
...
Η Άσκηση 4 ολοκληρώθηκε με επιτυχία, επιτυγχάνοντας τον τελικό στόχο της ενσωμάτωσης όλων των επιμέρους μονάδων σε έναν ολοκληρωμένο επιταχυντή \textlatin{AI}. Η σχεδίαση βασίστηκε σε μια στιβαρή \textlatin{Moore FSM} 7 καταστάσεων, η οποία διαχειρίζεται ορθά τον συγχρονισμό του \textlatin{datapath}, τη φόρτωση δεδομένων από τη \textlatin{ROM} και την υλοποίηση του \textlatin{pipeline} 5 κύκλων.
\\\\
Το σύστημα ανταποκρίνεται πλήρως στις αναθεωρημένες οδηγίες, απαιτώντας το σήμα \texttt{\textlatin{enable}} για την εκκίνηση της διαδικασίας φόρτωσης και αποδίδοντας την τιμή -1 (\texttt{\textlatin{32'hFFFFFFFF}}) σε περιπτώσεις υπερχείλισης. Παρά την ελάχιστη στατιστική ασυμφωνία (\texttt{\textlatin{298/300 tests passed}}) που οφείλεται αποκλειστικά στον σχεδιαστικό περιορισμό της \textlatin{ALU} ως προς την ανίχνευση υπερχείλισης σε πράξεις ολίσθησης κατά το τελικό στάδιο, η συνολική λειτουργία του κυκλώματος κρίνεται απόλυτα ορθή.
